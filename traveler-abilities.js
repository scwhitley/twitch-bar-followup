...
function fmt(scores) {
  ...
  const line = (abbr) => {
    const mod = mods[abbr.toLowerCase()] ?? 0;
    const sign = mod >= 0 ? "+" : "−";
    return `**${abbr}** ${scores[abbr]} (${sign}${Math.abs(mod)})`;
  };
  return ["STR", "DEX", "CON", "INT", "WIS", "CHA"].map(line).join("  •  ");
}
...
// Robust interaction updater so we don’t get “This interaction failed.”
...
export async function onMessageCreate(msg) {
  ...
  // Reset abilities (self; admins can target someone else)
  if (cmd === "!abilreset") {
...
export async function onInteractionCreate(ix) {
  ...
  if (ix.customId === "trav:abil:lock") {
    ...
    return void safeUpdate(ix, {
      embeds: [e],
      components: rows(true, rer), // disabled buttons now
    });
  }
