...
app.get("/marvel-overlay", (req, res) => {
  res.send(`
    ...
    <head>
      ...
      <style>
        ...
        #bar-fill {
          height: 100%;
          width: 0%;
          background: linear-gradient(90deg, #ff2ce6, #b300ff);
          box-shadow: 0 0 15px rgba(179,0,255,0.9);
    ...
    <body>
      <div id="tracker">
        <h2>Scarlet Witch – Centurion Grind</h2>
        <div id="bar-container">
          <div id="bar-fill"></div>
        </div>
        <div id="numbers">
          <span id="current">0</span> / <span id="goal">1200</span>
        </div>
      </div>
      ...
      <script>
        async function updateTracker() {
          try {
            ...
            const percent = Math.max(0, Math.min(100, (data.current / data.goal) * 100));
            barFill.style.width = percent + "%";
          } catch (err) {
            console.log("Marvel overlay error:", err);
          }
        }
